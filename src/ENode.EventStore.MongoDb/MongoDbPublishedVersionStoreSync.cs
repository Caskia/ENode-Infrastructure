...
{
    public class MongoDbPublishedVersionStoreSync : IPublishedVersionStore
    {
        ...
        public Task<AsyncTaskResult<int>> GetPublishedVersionAsync(string processorName, string aggregateRootTypeName, string aggregateRootId)
        {
            try
            {
                ...
                var result = _publishedVersionStoreCollection.GetCollection(aggregateRootId).Find(filter).ToList();
                var version = result.Select(r => r.Version).SingleOrDefault();

                return Task.FromResult(new AsyncTaskResult<int>(AsyncTaskStatus.Success, version));
            }
            catch (MongoQueryException ex)
            {
                _logger.Error("Get aggregate published version has query exception.", ex);
                return Task.FromResult(new AsyncTaskResult<int>(AsyncTaskStatus.IOException, ex.Message));
            }
            catch (Exception ex)
            {
                _logger.Error("Get aggregate published version has unknown exception.", ex);
                return Task.FromResult(new AsyncTaskResult<int>(AsyncTaskStatus.Failed, ex.Message));
            }
        }

        public MongoDbPublishedVersionStoreSync Initialize(
        ...
        public Task<AsyncTaskResult> UpdatePublishedVersionAsync(string processorName, string aggregateRootTypeName, string aggregateRootId, int publishedVersion)
        {
            if (publishedVersion == 1)
            {
                ...
                try
                {
                    _publishedVersionStoreCollection.GetCollection(aggregateRootId).InsertOne(record);
                ...
                catch (MongoWriteException ex)
                {
                    ...
                    return Task.FromResult(new AsyncTaskResult(AsyncTaskStatus.IOException, ex.Message));
                }
                catch (Exception ex)
                {
            ...
            else
            {
                try
                {
                    ...
                    _publishedVersionStoreCollection.GetCollection(aggregateRootId)
